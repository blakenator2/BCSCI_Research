...
TRAIN_DAYS = list(range(1283, 1303))
TEST_DAYS  = list(range(1305, 1325))
...
def create_roi_mask(df, day, shape):
    mask = np.zeros(shape, dtype=np.int8)  # Use int8 instead of int
...
def get_region_vectorized(x, y):
    """Vectorized region computation"""
    regions = np.empty(len(x), dtype=object)
    ...
    # Assign regions
    regions[row_nw_n_ne & col_left] = "NW"
    regions[row_nw_n_ne & col_mid] = "N"
    regions[row_nw_n_ne & col_right] = "NE"
    
    regions[row_w_c_e & col_left] = "W"
    regions[row_w_c_e & col_mid] = "C"
    regions[row_w_c_e & col_right] = "E"
    
    regions[row_sw_s_se & col_left] = "SW"
    regions[row_sw_s_se & col_mid] = "S"
    regions[row_sw_s_se & col_right] = "SE"
    
    return regions

def tex(mat):
    a = ((mat - mat.min())/(np.ptp(mat)+1e-6)*255).astype(np.uint8)
    G = graycomatrix(a,[1],[0],levels=256,symmetric=True,normed=True)
    return [graycoprops(G,k)[0,0] for k in
            ("contrast","dissimilarity","homogeneity","energy")]

# === OPTIMIZED COLLECTION WITH BATCHING ===
...
for day in ALL_DAYS:
    ...
    # Pre-compute Sobel gradients for entire grids
    ...
    for xr in xr_coords:
        for yr in yr_coords:
            ...
            # Compute region once per point
            if yr >= 50:
                region = "NW" if xr < 25 else "N" if xr < 50 else "NE"
            elif yr >= 25:
                region = "W" if xr < 25 else "C" if xr < 50 else "E"
            else:
                region = "SW" if xr < 25 else "S" if xr < 50 else "SE"
            ...
            for corner in ("NW","NE","SW","SE"):
                ...
                # Get pre-computed gradients at center
                psx = P_sobel_x[row, xr]
                psy = P_sobel_y[row, xr]
                wsx = W_sobel_x[row, xr]
                wsy = W_sobel_y[row, xr]
...
print(f"Collected {len(img_patches)} patches")
